...
# Cargar los fotogramas de la animación de destrucción
destruction_frames = [pygame.image.load(f"destruction_frame_{i}.png") for i in range(1, 6)]
destruction_frames = [pygame.transform.scale(frame, (brick_width, brick_height)) for frame in destruction_frames]
...
class Brick:
    def __init__(self, rect):
        ...
        self.destruction_frame = 0

    def draw(self, screen):
        if self.destroyed:
            if self.destruction_frame < len(destruction_frames):
                screen.blit(destruction_frames[self.destruction_frame], self.rect.topleft)
